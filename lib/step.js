...
// add events to the Step object
Step.prototype.__proto__ = emitter;
...
Step.prototype.makeRequest = function() {
  ...
  this.emit('step:pre-request', params);
...
Step.prototype.handleInitialResponse = function(err, res, body) {
  ...
  if (err) this.emit('step:post-request', err);
  ...
  this.emit('step:post-request', null, res);
...
Step.prototype.handleCallbackRequest = function(req, cb) {
  ...
  this.emit('step:post-callback', req)
